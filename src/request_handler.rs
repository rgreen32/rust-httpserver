...
use std::{fs::File, io::{self, BufRead, BufReader, Read, Write}, net::{TcpStream}, path::Path, time::Duration};
...
pub fn handle_request(request: HttpRequest) -> HttpResponse {
    ...
    let (status_code, reason_phrase, headers, response_body): (Option<u32>, Option<String>, Option<HashMap<String, String>>, Option<String>) = match path_components[0] {
                                                                                                                                                    "" => {
                                                                                                                                                        let status_code = 200;
    ...
    let response: HttpResponse = match status_code {
        Some(status_code) =>  HttpResponse {
                                    ...
                                    body: response_body.unwrap_or(String::new())
                                },
        None =>  HttpResponse {
                        version: request.request_line.version,
...
pub fn read_stream_into_request<S: BufRead>(stream: &mut S) -> Result<HttpRequest, io::Error> {
...
pub fn deserialize_requestline(request_line_string: String) -> RequestLine {
    let request_line_components: Vec<String> = request_line_string.split(" ")
                                                                  .map(|s| s.to_string())
                                                                  .collect();
